            ...
            <div className="flex flex-col xl:flex-row gap-8 justify-between">
              ...
              <div className="flex-1">
                ...
                <div className="mt-8 space-y-3">
                  ...
                  {camp.adItems.map(ad =>(
                    <div key={ad.id} className="rounded-xl border border-[#222] bg-[#0a0a0a] overflow-hidden">
                      {/* OG Visual Preview — WhatsApp style */}
                      {ad.previewImage ? (
                        <div className="h-36 w-full bg-[#111]">
                          <img src={ad.previewImage} alt="Ad preview" className="h-full w-full object-cover" />
                        </div>
                      ) : (
                        <div className="flex h-20 items-center justify-center bg-[#111] text-gray-700 text-xs font-mono">No preview</div>
                      )}
                      <div className="p-3">
                        {ad.ogTitle && <p className="text-xs font-semibold text-white line-clamp-1 mb-0.5">{ad.ogTitle}</p>}
                        {ad.ogDescription && <p className="text-[11px] text-gray-500 line-clamp-2 mb-2">{ad.ogDescription}</p>}
                        <p className="text-[10px] font-mono text-[#6d28d9] truncate mb-2">{ad.postUrl}</p>
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`text-[10px] uppercase font-mono px-1.5 py-0.5 rounded border ${
                            ad.status === 'ACTIVE'   ? 'text-emerald-400 border-emerald-900/50 bg-emerald-950/20' :
                            ad.status === 'APPROVED' ? 'text-sky-400 border-sky-900/50 bg-sky-950/20' :
                                                       'text-gray-500 border-[#333] bg-[#111]'
                          }`}>{ad.status}</span>
                          <span className="text-[10px] font-mono text-gray-600 ml-auto">${ad.itemBudget}</span>
                        </div>
                      </div>
                    </div>
                  ))}
